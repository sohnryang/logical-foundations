...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1, s2 with
  | nil, _ => true
  | n1 :: t1, _ => andb (member n1 s2) (included t1 (remove_one n1 s2))
  end.
...
Theorem add_inc_count : forall s n, count n (add n s) = S (count n s).
...
Proof.
  intros. induction s1 as [| m s1' IHs1'].
  - simpl. reflexivity.
  - simpl. destruct (n =? m) eqn:Enm.
...
Proof.
  intros.
  assert(H0: l1 = rev (rev l1)).
  { rewrite rev_involutive. reflexivity. }
  assert(H1: l2 = rev (rev l2)).
  { rewrite rev_involutive. reflexivity. }
  rewrite H0. rewrite H1. rewrite H. reflexivity. Qed.
(** [] *)
