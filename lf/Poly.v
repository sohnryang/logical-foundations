...
(* FILL IN HERE
    ...
    [] *)
Compute fold (fun n l => n * 2 :: l) [0; 1; 2; 3] [].
...
(*
  ...
  l == [] : l is nil, so nth_error l n is None.
  l != [] : let l = x :: l' and suppose nth_error l' (length l') = None.
            Since l is not empty, nth_error l n = nth_error l' (pred n).
            length l' = pred n, so nth_error l n = None.
...
Compute two nat S.
Compute two nat S O.
Compute two nat (two nat S).
Compute two nat (two nat S) O.
Compute two nat (two nat (two nat S)).
Compute two nat (two nat (two nat S)) O.
Compute two nat (two nat (two nat (two nat S))).
Compute two nat (two nat (two nat (two nat S))) O.

Compute three (nat -> nat) (two nat).
Compute three (nat -> nat) (two nat) S.
Compute three (nat -> nat) (two nat) S O.
Compute two (nat -> nat) (two nat) S O.
